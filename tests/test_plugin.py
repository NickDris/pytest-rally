...
class TestPlugin:
    ...
    def test_track_filter_limits_autogenerated_tracks(self, pytester, example, temp_repo):
        def expected_test_names(track_filter):
            filter_items = "" if track_filter == "" else[t.strip() for t in track_filter.split(",")]
            if not filter_items:
                result = [
            ...
            elif all(f not in self.tracks for f in filter_items):
                result = [f"test_track_challenge[track0-challenge0-rally_options0]"]
            else:
                result = [
                    f"test_track_challenge[{track}-{challenge}]"
    ...
    def test_track_filter_skips_tracks(self, caplog, temp_repo, example, run_with_filter):
        def expected_tracks_filtered(track_filter):
            ...
            if not filter_items:
                ...
                return set([track for track in self.tracks if track in filter_items])
