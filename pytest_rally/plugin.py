    ...
    if current_class is not None and current_class.__name__ == desired_class:
        if "track" and "challenge" in metafunc.fixturenames:
            ...
            for track, challenges in tracks_and_challenges:
                if not track_filter or any(track==tf or track.split("/")[0]==tf for tf in track_filter):
    ...
    for item in items:
        ...
        if track_filter:
            ...
            if track_marker:
                ...
                if not any(
                    marker == tf or marker.split("/")[0] == tf
                    for marker in marker_tracks
                    for tf in track_filter
                ):
